...
To create this, we need an API key from an LLM provider.  As of this writing, OpenAI provides keys even on free accounts.  Billing is on a per-use basis, and it is miniscule on demonstration scales.  As I will explain, there are also API keys for LLMs that are completely free.  Julius provides a facility for storing an API key as a "secret key."  It is not exposed to the LLM during the coding that creates the custom chatbot.  
