...
"Seeking alpha" is a common phrase and goal in finance (for example, [Seeking Alpha](https://seekingalpha.com/) is a popular website among investors).  It has its origin in mean-variance analysis and the CAPM.  Alpha is the intercept in the CAPM regression.  It was first used to measure mutual fund performance by Michael Jensen, and, when used in that context, it is called Jensen's alpha.  The goal is to generate positive alpha.
...
In an [earlier post](https://finance-with-ai/02-capm), I explained how to use Julius to run the CAPM regression, using stock prices from Yahoo and market and risk-free returns from [French's data library](https://mba.tuck.dartmouth.edu/pages/faculty/ken.french/data_library.html).  We can do the same with a mutual fund ticker.  Rather than using the most recent 60 months as in the earlier post, I prefer to run the regression for this exercise using all available data.  Then, we can ask Julius to compute the beta-hedged mutual fund return each month.  By beta hedging, I mean taking out the market exposure as
R - beta * (Rm - Rf).  According to the CAPM, the average value of this beta-hedged return should be the risk-free return Rf.  We can ask Julius to compound the risk-free returns and compound the beta-hedged mutual fund returns over time and plot them.  If the beta-hedged returns outperform the risk-free returns, then the fund is earning alpha.  It is useful to ask for the plot with a log scale on the y axis.  Then, we can easily see the periods in which the fund was generating positive alpha (the slope of the hedged fund curve is greater than the slope of the risk-free curve) and the periods in which it was generating negative alpha.

A good example for this exercise is Fidelity Magellan (ticker=FAMGX).  It is remarkable to see how much alpha the fund earned under the stewardship of Peter Lynch and how much negative alpha it produced afterwards.
---
