...
There are python tools for executing the JavaScript code and retrieving the data, in particular the selenium library, but I have never had the patience to complete that type of task.^[It is not obvious that Julius is the best tool for this anyway, because it seems impossible to install the chromium webdriver and there seem to be some limitations of the available firefox webdriver.]  Perhaps AI agents using Anthropic's [Model Context Protocol (MCP)](https://www.anthropic.com/news/model-context-protocol) to control a web browser are the best tool for that type of exercise now.  More on that later.
