...
As a next step, students can ask Gemini to create a custom chatbot using Streamlit and ngrok.  A good example for a system prompt is to ask the LLM to respond in a foreign language.  Then, students can see that the system prompt actually works.  If the chatbot does not seem to be remembering past prompts during a session, tell Gemini to create a loop that collects all past prompts and responses and to send all past prompts and responses together with the system prompt with each new prompt.  

After successfully building chatbots, students are ready to start building finance chatbots.  For a first finance chatbot, use the investment report example from the course introduction.  But, I would focus on the DCF analysis.  
