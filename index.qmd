...
Students can use any chatbot for the first part of the course.  Then, the course switches to using Google Colab, which is a free Python environment in the cloud that has built-in Google Gemini assistance.  There is no software to install, so there is no set-up required.  I some self-guided instructions for [using Google Colab and Jupyter notebooks](https://kerryback.substack.com/p/07-mean-variance-analysis).

For the last part of the course, students need an OpenAI account.  They can use the free version.  They will need to get an API key, for which charges are on a per-usage basis.  The charges are miniscule for the experimentation that is done in the course.  
...
6. In-depth study of using AI for DCF valuation of companies
