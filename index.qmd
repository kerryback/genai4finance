...
1. We should treat AI as a colleague, collaborator, and advisor, as well
as an assistant - - see Ethan Mollick, [Good Enough Prompting](https://www.oneusefulthing.org/p/getting-started-with-ai-good-enough). 
2. Large language models (LLMs) cannot be relied upon to do
arithmetic, so coding (Python) is essential.
3. AI + coding (vibe coding) can perform many financial analyses as
well as or better than Excel.
4. For critical operations, we should save tested code as an app.
5. Chatbots can be customized through the system prompt, retrieval
augmented generation (RAG) or fine tuning.
6. To use a chatbot for critical operations, we should create an app and
configure it as a chatbot tool, creating an AI agent.
...
Previously, I used [Julius.ai](https://julius.ai) instead of Claude. Julius provides access to Claude models, in addition to OpenAI models.  It provides a 50% academic discount, which is an advantage, but it does not provide Excel generation or MCP connections.  ChatGPT is another possibility. It has its Assistants API and Custom GPTs as an alternative to MCP connections. It can also generate Excel files, but in my experience Claude is currently doing a better job.  Furthermore, ChatGPT provides only a limited (though standard) set of Python libraries, unlike Claude, which allows users to add whatever Python libraries they need to the environment that Claude provides in the cloud. 
