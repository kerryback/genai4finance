...
Previously, I used [Julius.ai](https://julius.ai).  It is a bit simpler to use, because it is a simple chatbot interface rather than a Jupyter notebook - see my [blog post about Julius](https://kerryback.substack.com/p/07-mean-variance-analysis).  Julius also provides access to LLMs from OpenAI and Anthropic, whereas Colab only offers Google Gemini.  However, while Anthropic's Claude is still the best coding LLM, Gemini has caught up considerably and is now a solid choice.  Furthermore, Colab offers the following advantages: it is free, it produces Jupyter notebooks that are portable, and it can deploy apps to the cloud. 
