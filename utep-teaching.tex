...
\vspace{1cm}
\includegraphics[width=\textwidth]{mgmt675-slides/images/slide10_img1.png}
\end{column}
\end{columns}
\end{frame}
...
\vspace{0.3cm}
\begin{shadedbox}
\centerline{\href{https://mgmt675.kerryback.com/exercises/betas.tex}{Data for Exercise}}
\end{shadedbox}
\end{frame}
...
\begin{baritemize}
  \item Claude ``skills'' are instructions sent to Claude when needed
...
\vfill
\begin{center}
{\fontsize{36}{40}\selectfont\bfseries\color{titlegray} Creating Custom Chatbots}
\end{center}
\vfill
\end{frame}
...
\begin{frame}{System Prompts Differ by Platform}
\begin{columns}[T]
\begin{column}{0.5\textwidth}
\begin{shadedbox}[title=\textbf{Claude.ai / Desktop}]
\begin{itemize}\small
  \item Anthropic injects a system prompt
  \item Provides current date, behavioral guidelines
  \item Published and periodically updated
\end{itemize}
\end{shadedbox}
\vspace{0.3cm}
\begin{shadedbox}[title=\textbf{API}]
\begin{itemize}\small
  \item \alert{No system prompt by default}
  \item Developer provides their own
  \item Full control over behavior
\end{itemize}
\end{shadedbox}
\end{column}
\begin{column}{0.5\textwidth}
...
\begin{itemize}\small
  \item Has its own extensive system prompt
  \item Covers tool usage, git safety, coding practices
  \item Maintained by Claude Code team
\end{itemize}
\end{shadedbox}
\vspace{0.3cm}
\begin{shadedbox}[title=\textbf{Custom Chatbots}]
\begin{itemize}\small
  \item You write the system prompt
  \item Defines persona, expertise, rules
  \item This is the key to customization
\end{itemize}
\end{shadedbox}
\end{column}
\end{columns}
...
% ========================================================================
% IMPORTED DECK: Creating AI Agents
% ========================================================================

\begin{frame}[plain]
\vfill
\begin{center}
{\fontsize{36}{40}\selectfont\bfseries\color{titlegray} Creating AI Agents}
\end{center}
\vfill
\end{frame}
...
\begin{enumerate}\small
  \item Write SQL to identify top 5 customers by revenue (LLM)
  \item Execute the SQL query against the database
  \item Write SQL to get quarterly revenue for those customers (LLM)
  \item Execute that query
  \item Write Python to analyze trends and create visualizations (LLM)
  \item Execute the Python code
  \item Analyze the results (LLM)
  \item Write Python to generate a written report for the user (LLM)
  \item Execute the Python code
  \item Send the report to the user
\end{enumerate}
\end{shadedbox}
\end{frame}
...
\vfill
\begin{center}
{\fontsize{26}{30}\selectfont\bfseries\color{titlegray} More for Students and for Us}
\end{center}
\vfill
\end{frame}
...
\centering\href{https://kerryback.com/mgmt675/exercises/betas.xlsx}{Data for Exercise}
\end{shadedbox}
\end{center}
\end{frame}
...
\begin{center}
\begin{shadedbox}[width=0.7\textwidth]
\centering\href{https://kerryback.com/mgmt675/exercises/aggregation.zip}{Data for Exercise}
\end{shadedbox}
\end{center}
\end{frame}
